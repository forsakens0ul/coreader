...
export function ReadingInterface({ book, onClose }: ReadingInterfaceProps) {
  const { settings, updateSettings, highlights, addHighlight, updateBook, notes, addNote } = useBookStore()
  ...
  useEffect(() => {
    if (pageInfo) {
      const progress = Math.round((pageInfo.globalPage / pageInfo.totalPages) * 100)
      updateBook(book.id, { 
  ...
  }, [settings.fontSize, settings.fontFamily, settings.lineHeight, settings.pageWidth])
  ...
  const createHighlight = (color: 'yellow' | 'blue' | 'green' | 'red', style: 'highlight' | 'underline' | 'wave') => {
  ...
  const shareText = () => {
    if (selectedText && pageInfo) {
      const shareText = `"${selectedText}" - ${book.title} by ${book.author} (Page ${pageInfo.globalPage + 1})`
      if (navigator.share) {
        navigator.share({
          title: book.title,
          text: shareText
  ...
  const SettingsPanel = () => (
    <motion.div
      ...
      className="fixed right-0 top-0 h-full w-80 bg-background border-l shadow-xl z-50 overflow-hidden"
    >
      <div className="p-6 h-full overflow-y-auto">
        ...
        <Tabs defaultValue="display" className="w-full">
          ...
          <TabsContent value="display" className="space-y-6 mt-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm text-muted-foreground">{settings.pageWidth}px</span>
              </div>
              <Slider
                value={[settings.pageWidth]}
                onValueChange={([value]) => updateSettings({ pageWidth: value })}
                min={400}
                max={1000}
                step={50}
                className="w-full"
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  { value: 'paper', label: 'Paper', bg: 'bg-orange-50', text: 'text-orange-900' },
                ].map((theme) => (
                  <Button
                    key={theme.value}
                  ...
                  >
                    <div className={cn('w-4 h-4 rounded-full border', theme.bg, theme.text)} />
                    {theme.label}
                  </Button>
                ))}
              </div>
          ...
          <TabsContent value="reading" className="space-y-6 mt-6">
            {/* Page Transition */}
            <div>
              <label className="text-sm font-medium mb-3 block">Page Transition</label>
              <Select
                value={settings.pageTransition}
                onValueChange={(value: any) => updateSettings({ pageTransition: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <ScrollArea className="h-full">
          <div className="space-y-2">
            {readingEngine.getChapters().map((chapter) => (
              <Button
                key={chapter.id}
                variant={pageInfo?.chapterId === chapter.id ? 'default' : 'ghost'}
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Pages {chapter.startPage + 1} - {chapter.endPage + 1} ({chapter.pageCount} pages)
                  </p>
                </div>
              </Button>
            ))}
          ...
          <ScrollArea className="max-h-60">
            <div className="space-y-2">
              {searchResults.map((result, index) => (
                <Card key={index} className="p-3 cursor-pointer hover:bg-muted/50" onClick={() => goToSearchResult(result.globalPage)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="text-sm font-medium">{result.chapterTitle}</p>
    ...
    <AnimatePresence>
      {showHighlightMenu && (
        <motion.div
          ...
          style={{
            left: Math.max(10, Math.min(highlightMenuPosition.x - 100, window.innerWidth - 210)),
        ...
        >
          <div className="flex items-center gap-1 mb-2">
            <Button
              ...
              className="bg-yellow-200 hover:bg-yellow-300 w-8 h-8 p-0"
            ...
            <Button
              size="sm"
              variant="ghost"
              onClick={() => createHighlight('blue', 'highlight')}
              className="bg-blue-200 hover:bg-blue-300 w-8 h-8 p-0"
            ...
            <Button
              size="sm"
              variant="ghost"
              onClick={() => createHighlight('green', 'highlight')}
              className="bg-green-200 hover:bg-green-300 w-8 h-8 p-0"
            ...
            <Button
              size="sm"
              variant="ghost"
              onClick={() => createHighlight('red', 'highlight')}
              className="bg-red-200 hover:bg-red-300 w-8 h-8 p-0"
              title="Red Highlight"
            >
              <Highlighter className="h-3 w-3" />
            </Button>
          ...
          <div className="flex items-center gap-1">
            <Button size="sm" variant="ghost" onClick={() => createHighlight('yellow', 'underline')} title="Underline">
              <Underline className="h-3 w-3" />
            </Button>
            <Button size="sm" variant="ghost" onClick={() => createHighlight('yellow', 'wave')} title="Wave Underline">
              <Waves className="h-3 w-3" />
            </Button>
            <Button size="sm" variant="ghost" onClick={openNoteDialog} title="Add Note">
  ...
  return (
    <div className={cn('min-h-screen reading-container', themeClass, isFullscreen && 'fixed inset-0 z-50')}>
      {/* Header */}
      {!isFullscreen && (
        <div className="sticky top-0 z-40 bg-background/95 backdrop-blur border-b">
          <div className="flex items-center justify-between p-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={() => setShowSearch(true)} title="Search">
                <Search className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setShowChapters(true)} title="Table of Contents">
      ...
      {isFullscreen && (
        <div className="fixed top-4 right-4 z-50 flex gap-2">
          <Button variant="outline" size="sm" onClick={toggleFullscreen} title="Exit Fullscreen">
            <Minimize className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={() => setShowSettings(true)} title="Settings">
            <Settings className="h-4 w-4" />
          </Button>
        </div>
      )}
      ...
      <div className="relative flex-1">
        ...
        >
          <AnimatePresence mode="wait">
            ...
            >
              <div className="mb-6">
                <h2 className="text-xl font-semibold mb-2">{pageInfo.chapterTitle}</h2>
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                  <span>Page {pageInfo.pageInChapter + 1} of {pageInfo.totalPagesInChapter}</span>
                  <span>•</span>
                  <span>Chapter {pageInfo.globalPage + 1} of {pageInfo.totalPages}</span>
                </div>
              </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-4">
                {isReading ? (
                  ...
                  <Button variant="outline" size="sm" onClick={startAutoReading}>
            ...
            <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
              <motion.div
                ...
                initial={{ width: 0 }}
                animate={{ width: `${((pageInfo.globalPage + 1) / pageInfo.totalPages) * 100}%` }}
                transition={{ duration: 0.3 }}
              />
            </div>
          </div>
