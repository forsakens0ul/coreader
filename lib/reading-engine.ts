...
export class ReadingEngine {
  ...
  getPageInfo(pageIndex: number): PageInfo {
    ...
    const chapter = this.book.chapters.find(
      (ch) => pageIndex >= ch.startPage - 1 && pageIndex < ch.endPage
    );
  ...
  searchText(query: string): Array<{
    ...
    for (let i = 0; i < this.pages.length; i++) {
      ...
      if (pageContent.toLowerCase().includes(queryLower)) {
        ...
        // 获取该页所在章节信息
        const pageInfo = this.getPageInfo(i);
...
const useReadingEngine = create<ReadingEngineState>((set, get) => ({
  ...
  setPage: (page: number) => {
    ...
    if (page !== currentPage) {
      ...
      if (currentBook) {
        const newChapter = currentBook.chapters.findIndex(
    ...
    if (
      ...
      if (currentBook) {
        ...
        const newPage = Math.max(
          1,
          Math.min(Math.round(currentPageProgress * pages.length), pages.length)
        );
...
function simpleSplitIntoPages(
  ...
  const charsPerLine = Math.floor(
    (options.pageWidth - 2 * options.padding) / (options.fontSize * 0.6)
  );
  const linesPerPage = Math.floor(
    (options.pageHeight - 2 * options.padding) /
      (options.fontSize * options.lineHeight)
  );

  // 考虑中文和英文混排的情况
  const isMostlyChinese =
    (content.match(/[\u4e00-\u9fff]/g) || []).length > content.length * 0.5;
    ...
    for (const paragraph of paragraphs) {
      ...
      if (paragraph.length > charsPerPage * 1.5) {
        ...
        for (const sentence of sentences) {
          if (
            currentLength + sentence.length > charsPerPage &&
            currentLength > 0
          ) {
...
function canvasSplitIntoPages(
  ...
  // 检测是否主要是中文内容
  const isMostlyChinese =
    (content.match(/[\u4e00-\u9fff]/g) || []).length > content.length * 0.3;
