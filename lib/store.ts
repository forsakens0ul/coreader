      ...
      addBook: (bookData) => {
        ...
        try {
          ...
          if (
            ...
            console.log(
              `书籍"${bookData.title}"内容长度: ${bookData.content.length}`
            );
            ...
            if (bookData.content.length > maxContentLength) {
              console.warn(
                `书籍内容过大(${bookData.content.length}字符)，截断到${maxContentLength}字符`
              );
              bookData.content = bookData.content.substring(
                0,
                maxContentLength
              );
            }
            ...
            for (let i = 0; i < bookData.chapters.length; i++) {
              ...
              if (chapter.content && chapter.content.trim() !== "") {
                ...
                if (chapter.content.length > maxChapterLength) {
                  console.warn(
                    `章节 "${chapter.title}" 内容过大(${chapter.content.length}字符)，截断到${maxChapterLength}字符`
                  );
                  bookData.chapters[i].content = chapter.content.substring(
                    0,
                    maxChapterLength
                  );
                }
      ...
      removeBook: (bookId) => {
        set((state) => ({
          ...
          highlights: state.highlights.filter(
            (highlight) => highlight.bookId !== bookId
          ),
